      ...
      <div className='max-w-6xl mx-auto p-6'>
        ...
        <div className='inline-flex flex-wrap items-center border border-gray-200 rounded-md p-1 bg-white shadow-sm'>
          {
            dataArray.map((tab)=> (
              <button onClick={()=> setCurrentTab(tab.label)}
               key={tab.label} className={`cursor-pointer flex items-center px-3 py-1 text-sm rounded-md transition-colors
        ...
        <div className='flex flex-wrap gap-6 mt-6'>
          {dataArray.find((item)=> item.label === currentTab).value.map((user)=> (
             ...
             shadow rounded-md'>
              <img src={user.profile_picture} alt="" className="rounded-full w-12 h-12
              shadow-md mx-auto" />
